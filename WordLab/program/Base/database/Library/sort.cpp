int sortlib(int bgn,int end,const string admin,const string pwd) {
	...
	for(i=bgn; i<=end; i++) {
		//validate
		if(!LibraryAdminValidate(i,EN,admin,pwd)) {
            return sortLibError(-7,admin);
        }
		//read
		if((ln=filelines(LibraryRoute(i,EN),1))<=0) {
            return sortLibError(-1);
        }
		if((s_en=(char **)malloc(sizeof(char*)*ln))==NULL) {
		...
		if((s_ch=(char **)malloc(sizeof(char*)*ln))==NULL) {
            return sortLibError(-2);
        }
		for(int r=0; r<ln; r++) {
		...
		if(loadmsg(s_en,LibraryRoute(i,EN),ln,32,1,0)<=0) {
            return sortLibError(-3);
        }
		if(loadmsg(s_ch,LibraryRoute(i,CH),ln,256,1,0)<=0) {
            return sortLibError(-3);
        }
		//sort
		...
		//record
		FILE *fp_en, *fp_ch;
		if((fp_en=fopen("lib_en.dat","w"))==NULL) {
            return sortLibError(-5);
        }
		if((fp_ch=fopen("lib_ch.dat","w"))==NULL) {
            return sortLibError(-5);
        }
		for(int r=0; r<ln; r++) {
		...
		if(rename("lib_en.dat",LibraryRoute(i,EN).c_str())!=0) {
            return sortLibError(-6);
        }
		remove(LibraryRoute(i,CH).c_str());
		if(rename("lib_ch.dat",LibraryRoute(i,CH).c_str())!=0) {
            return sortLibError(-6);
        }
		//free memory
		for(int r=0; r<ln; r++) {
