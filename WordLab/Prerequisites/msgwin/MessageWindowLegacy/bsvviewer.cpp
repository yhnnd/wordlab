int MessageWindow::bsvviewer(const char *route,bool _Animation,int x,int y,int w,int h,bool select) {
	...
	if(_Animation) {
        Show(x,y,width,&msgs[0][0],height,linemax);
    } else  {
        Frame(0,x,y,width,&msgs[0][0],height,linemax);
    }
	...
	if (_Animation) {
        Hide(x,y,width,&msgs[0][0],height,linemax);
    } else {
        Frame(-1,x,y,width,&msgs[0][0],height,linemax);
    }
