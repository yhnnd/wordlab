int scriptshell(std::string filename,int print_debug_msg,int data_capacity,COORD pos,int w,int h) {
	...
	if(filename=="new") { //new file
		...
		if(system((host::name+" ScriptEditor "+filename).c_str())) return 1;
	...
	if(l1==std::string::npos and l2==std::string::npos) logfolder = "";
	else {
		if(l1==std::string::npos) len=l2;
		else if(l2==std::string::npos) len=l1;
		else len = l1 > l2 ? l1 : l2;
	...
	for(;;) {
		...
		for(auto e:functname) std::cout<<"("<<i++<<") function "<<e<<std::endl;
		std::cout<<"select function"<<std::endl;
		std::cout<<">>> ";
		std::cin>>tempstr;
		i = 0;
		if(isdigit(tempstr[0])) i=stoi(tempstr);
		else for(auto e:functname) {
				if(e==tempstr) break;
				++i;
			}
