void MessageWindow::Change(int x,int y,int width,char *msg,int max,int w){
	...
	if(r1==r-1){status("msgwin: change cancelled");goto end;}
	r2=Select(x+1,y+1,r,r1,"[=]","[B]","[0]");
	if(r2==r-1){status("msgwin: change cancelled");goto end;}
	if(r1>r2) Selectup  (msg,max,w,r1,r2,r,s);
	else      Selectdown(msg,max,w,r1,r2,r,s);
