...
func (c *Client) readPump() {
	...
	for {
		...
		subStr := string(message)[equalsIndex+1:]

		if strings.HasPrefix(string(message), "username=") == true {
			username := subStr
			// setUsername sets the username of the client.
			c.setUsername(username)
			c.hub.broadcast <- []byte("new user " + username + " joined.")
		} else if strings.HasPrefix(string(message), "broadcast=") == true {
			...
			if strings.Contains(subStr, " ") == true {
				result := c.wlsearchphrase(subStr)
				if result.Found {
					c.send <- []byte("search phrase result: " + strconv.Itoa(result.Index) + " " + result.Message)
				} else {
					c.send <- []byte("search phrase: no result")
				}
			} else {
		...
		} else if strings.HasPrefix(string(message), "rollback=") == true {
			c.rollback(subStr)
		} else if strings.HasPrefix(string(message), "rollbackconfirm=") == true {
			c.rollbackconfirm(subStr)
